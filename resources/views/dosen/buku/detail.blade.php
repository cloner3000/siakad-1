...
<div class="box box-danger">
	<div class="box-header with-border">
		...
		<div class="box-tools">
			<a href="{{ route('dosen.buku.create') }}?dosen={{ $dosen -> id }}" class="btn btn-primary btn-xs btn-flat"><i class="fa fa-plus"></i> Tambah Buku</a>
		</div>
	</div>
	<div class="box-body">
		<table class="table table-bordered">
			...
			<tbody>
				@if(!$buku -> count())
				<tr>
					<td colspan="10" align="center">Belum ada data buku</td>
				</tr>
				@else
				<?php $c=1; ?>
				@foreach($buku as $b)
				<tr>
					...
					<td>{{ $b -> nama_buku }}</td>
					<td>{{ $b -> website_buku }}</td>
					<td>@if($b -> level_buku == 1) Nasional @else Internasional @endif</td>
					<td>{{ $b -> penerbit }}</td>
					<td>{{ $b -> issn }}</td>
					<td>@if($b -> akreditasi == 1) Sudah @else Belum @endif</td>
					<td>{{ $b -> tahun_terbit }}</td>
					<td>
						<a class="btn btn-warning btn-xs btn-flat" href="{{ route('dosen.buku.edit', $b -> buku_id) }}"><i class=" fa fa-edit"></i> Edit</a>
					</td>
				</tr>
				<?php $c++; ?>
				@endforeach
