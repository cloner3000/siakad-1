		...
		<table class="table table-bordered table-hover">
			...
			<tbody>
				...
				<tr @if($hari == $today)class="info" @endif >
					...
					<td class="matkuls">
						{{ $sem[$semester]['matkul'] }} Kelas {{ $sem[$semester]['kelas'] }}<br/>{{ $sem[$semester]['dosen'] }}
						<div class="opr">
							@if($sem[$semester]['id'])
							<a href="{{ route('matkul.tapel.jadwal.edit', $sem[$semester]['id']) }}" class="btn btn-xs btn-success" title="Ubah"><i class="fa fa-pencil"></i></a>
							<a href="{{ route('matkul.tapel.jadwal.delete', $sem[$semester]['id']) }}" class="btn btn-xs btn-danger has-confirmation" title="Hapus"><i class="fa fa-trash"></i></a>
							@else
							<a href="{{ url('/jadwal/create?id=' . $sem[$semester]['mtid']) }}" class="btn btn-xs btn-info" title="Baru"><i class="fa fa-plus"></i></a>
							@endif
						</div>
					</td>
					...
					<td class="matkuls"> -
						<div class="opr">
							<a href="{{ url('/jadwal/create?hari=' . $hari . '&jam_mulai=' . $waktu[0] . '&jam_selesai=' . $waktu[1]) }}" class="btn btn-xs btn-info" title="Baru"><i class="fa fa-plus-square-o"></i></a>
						</div></td>
						<td>-</td>
						@endif
						@endforeach
