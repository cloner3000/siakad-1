...
<script type="text/javascript">
	...
	$(document).on('click', '#btn-in', function(){
		...
		/* 
			if($('.from input:checkbox').length > 0)
			{
			$('.from input:checkbox').each(function(){
			me = $(this);
			fitem[m] = me.siblings('.txt').text();
			m++;
			});
			
			for(var c=0; c<fitem.length; c++ ) 
			{
			str = fitem[c].split(' - ');
			flist += '<div class="checkbox"><label><input type="checkbox" value="'+ str[0] +'" ><span class="number">' + (c+1) + '</span>. <span class="txt">' + fitem[c] + '</span></label></div>';
			}			
			$('.from').empty().append(flist); 
			} 
		*/
...
/* 	$(document).on('click', '#btn-process', function(){
		if($('.to input:checkbox').length < 1) return;
		$.ajax({
			url: '{{ url("/pengguna/resetpassword/mahasiswa/filter") }}',
			type: "post",
			data: {
				'target' : $('#target').val(),
