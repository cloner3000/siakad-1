...
<section class="content-header">
	<h1>
		Mahasiswa
		<small>Jenis Pembiayaan </small>
	</h1>
	<ol class="breadcrumb">
		<li><a href="/"><i class="fa fa-dashboard"></i> Home</a></li>
		<li><a href="{{ url('/mahasiswa') }}"> Daftar Mahasiswa</a></li>
		<li class="active">Jenis Pembiayaan </li>
	</ol>
</section>
@endsection
...
<script type="text/javascript">
	...
	$(document).on('click', '.btn-in', function(){
		...
		{
			$.ajax({
				url: '{{ url("/mahasiswa/adminpembiayaan") }}',
				type: "post",
				data: {
					'id' : id,
					...
					'jenis': $('select[name=\'jenis\'].select-to').val(),
					'_token': '{{ csrf_token() }}'
				},
				beforeSend: function()
				{
	...
	$(document).on('click', '.btn-filter-jenis', function (){
		var me = $(this);
		var target = me.attr('id').split('-')[1].trim();
		$.ajax({
			url: '{{ url("/mahasiswa/adminpembiayaan/anggota") }}',
			type: "post",
			data: {
				'jenis' : me.siblings('select[name=\'jenis\']' ).val(),
				'_token': '{{ csrf_token() }}'
			},
			beforeSend: function()
			{
			...
			<div class="col-sm-12">
				<div class="form-horizontal" role="form">
					<div class="row" style="margin-top: 3px;">
						...
						<div class="col-md-5">
							<div style="margin-top: 5px;">
								{!! Form::select('jenis', $jenis, null, ['class' => 'select-to form-control']) !!}
								<button class="btn btn-success btn-filter-jenis btn-flat" id="btn-to"><i class="fa fa-filter"></i></button>
								<div class="checkbox">
									<label>
