...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li><a href="{{ route('mahasiswa.pkm.peserta', $pkm -> id) }}"> Peserta PKM</a></li>
...
<section class="content-header">
	...
	<ol class="breadcrumb">
		<li><a href="/"><i class="fa fa-dashboard"></i> Home</a></li>
		<!--li><a href="{{ url('/pkm/info') }}"> Informasi PKM</a></li-->
		<li class="active">Pendaftaran PKM</li>
	</ol>
</section>
@endif
@endsection
...
<div class="box box-primary">
	<div class="box-header with-border">
		...
		@else
		<h3 class="box-title">Pendaftaran PKM</h3>
		@endif
	</div>
	<div class="box-body">
		@if($data == null)
		<div class="callout callout-danger">
			<h4>Kesalahan</h4>
			Pendaftaran PKM sudah ditutup. Silahkan menghubungi Bagian Akademik.
		</div>
		@else
		<div class="row">
			<div class="col-sm-3">
			...
			<div class="col-sm-9">
				{!! Form::model(new Siakad\Mahasiswa, ['class' => 'form-horizontal', 'role' => 'form', 'route' => ['mahasiswa.pkm.daftar']]) !!}
				{!! Form::hidden('foto', $data -> foto, array('id' => 'foto')) !!}
				<div class="form-group">
					<label class="col-sm-2 control-label">Periode PKM</label>
					<div class="col-sm-9">
						@if($show)
						<p class="form-control-static"><strong>{{ $data -> pkm -> nama }} ({{ formatTanggal(date('Y-m-d', strtotime($data -> pkm -> tanggal_mulai))) }} - {{ formatTanggal(date('Y-m-d', strtotime($data -> pkm -> tanggal_selesai))) }})</strong></p>
						@else
						{!! Form::select('pkm_id', $pkm, null, ['class' => 'form-control']) !!}
						@endif
					</div>
				</div>
				<div class="form-group">
				...
				@if($show)
				@if($admin)
				<a href="{{ route('mahasiswa.pkm.peserta.cetak', [$data -> pkm -> id, $data -> id]) }}" class="btn btn-info btn-flat" ><i class="fa fa-print"></i> Cetak</a>
				@else	
				<a href="{{ route('mahasiswa.pkm.peserta.cetak2') }}" class="btn btn-info btn-flat" ><i class="fa fa-print"></i> Cetak</a>
				@endif
				@else		
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
