...
<html lang="en">
	...
	<?php 
		...
		if(!$guest)
		{
			$rolename = !isset($rolename) ? strtolower(Auth::user() -> role -> name) : $rolename; 
						...
						<ul class="nav navbar-nav">					
							...
							<li class="dropdown user user-menu">
								<a href="#" class="dropdown-toggle" data-toggle="dropdown">
									...
									<span class="hidden-xs">{{ Auth::user() -> authable -> nama }}</span>
								</a>
								<ul class="dropdown-menu">
									<li class="user-header">
										...
										<p>
											{{ Auth::user() -> authable -> nama }} - {{ Auth::user() -> role -> name }}
											@if($rolename == 'mahasiswa')
											<small>{{ Auth::user() -> authable -> prodi -> nama }}</small>
											<small>{{ Auth::user() -> authable -> NIM }}</small>
											@else
											<small>Member since {{ date('M. Y', strtotime(Auth::user() -> authable -> created_at)) }}</small>
											@endif
										</p>
									</li>
