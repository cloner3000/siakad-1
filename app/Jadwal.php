<?php
	...
	class Jadwal extends Model
	{
		...
		public function scopeJadwalKuliah($query, $for = 'admin', $prodi_id = null, $ta_id = null)
		{			
			if($ta_id == 0) $ta_id = null;
			$query 
			...
			{
				$query 
				-> where('krs.mahasiswa_id', \Auth::user() -> authable -> id);
			}
			
			if($for == 'dosen') 
			{
				$query -> where('matkul_tapel.dosen_id', \Auth::user() -> authable -> id);
			}
			...
			'dosen.nama AS dosen', 'dosen.id AS dosen_id', 
			'kurikulum_matkul.semester', 
			'matkul_tapel.silabus', 'matkul_tapel.id as mtid', 'matkul_tapel.rpp', 'matkul_tapel.kelas2 AS kelas', 'matkul_tapel.id AS mtid', 
			'ruang.nama AS ruang', 
