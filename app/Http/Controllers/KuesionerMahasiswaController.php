	...
	class KuesionerMahasiswaController extends Controller
	{
		...
		*/
		public function update(Request $request, $id)
		{
			...
			{
				$kk = explode('-', $k);
				KuesionerMahasiswa::where('mahasiswa_id', $mhs_id) -> where('matkul_tapel_id', $id) -> where('kuesioner_id', $kk[1]) -> update(['skor' => $v]);
			}
		...
		public function penilaian($id = null, $mtid = null)
		{
			...
			$dosens = \DB::select("
			SELECT d.nama AS dosen, d.id AS iddosen, m.nama AS matakuliah, mt.id AS idmt, skor, t.nama as tapel FROM kuesioner_mahasiswa km
			...
			INNER JOIN dosen d ON d.id = mt.dosen_id
			INNER JOIN tapel t ON t.id = mt.tapel_id
			WHERE km.mahasiswa_id = :mhs_id AND km.matkul_tapel_id IN(". implode(', ', $mt_id) .")
			GROUP BY mt.id
			ORDER BY t.nama2
			",
			['mhs_id' => $mhs_id]
