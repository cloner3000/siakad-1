<?php namespace Siakad\Http\Controllers;
	...
	class BackupController extends Controller 
	{
		...
		{
			$name = env('DB_DATABASE') . '-' . date('YmdHis') . '.sql.gz';
		...
		{
			if(Input::get('_token') != csrf_token()) abort(404);
		...
		public function restore($id) 
		{
			...
			$sql = gzdecode(\File::get($storage -> getDriver() -> getAdapter() -> getPathPrefix() . $backup -> file));
